...
-->
	...
	<xsl:template match="n1:th">
        ...
        <xsl:template match="n1:td">
                        <xsl:variable name="code" select="following-sibling::*"/>
                        <td>
                                    <xsl:if test="xenc:EncryptedData">
                                    ...
                                    <xsl:if
                                                test="contains(., 'Acute HIV') or contains(., 'Substance Abuse')  or contains(., 'Persistent alcohol abuse') or contains(., 'Psychotic disorder') or contains(., 'Sickle cell anemia') or contains(., 'Alcohol dependence') or contains(., 'Anorexia nervosa') or contains(., 'Obsessive compulsive') or contains(., 'Acute stress disorder') or contains(., 'Major depressive disorder')">
                                                <xsl:variable name="externalDocId" select="ancestor::n1:section//n1:observation/n1:value[@code=$code]/n1:reference/n1:externalDocument/n1:text/n1:reference/@value"/>
                                                <span style="font-weight:bold;color:red">
                                                            <xsl:text>  </xsl:text>[ENTRY METADATA:<xsl:value-of select="$externalDocId"
                                                            />]</span>
                                    </xsl:if>
                                    <xsl:if test="contains(., 'bupropion') or contains(., 'Thorazine') or contains(., 'hydroxyurea') or contains(., 'Zoloft') or contains(., 'Zidovudine')">
                                                <xsl:variable name="externalDocId" select="ancestor::n1:section//n1:reference/n1:externalDocument/n1:text/n1:reference/@value"/>
                                                <span style="font-weight:bold;color:red">
                                                            <xsl:text>  </xsl:text>[ENTRY METADATA:<xsl:value-of select="$externalDocId"
                                                            />]</span>
